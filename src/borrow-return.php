...
<head>
    ...
    <style>
        .action-btn { padding: 5px 10px; border: none; border-radius: 6px; cursor: pointer; font-weight: 600; }
        .borrow-btn { background: #4CAF50; color: white; }
        ...
        <?php if($result && $result->num_rows>0): ?>
            <?php while($row=$result->fetch_assoc()): ?>
                <tr>
                    ...
                    <?php endif; ?>
                    <td>
                        <button class="action-btn <?= $mode==='borrow'?'borrow-btn':'return-btn'; ?>"
                                data-id="<?= $mode==='borrow'?$row['id']:$row['borrow_id']; ?>"
            ...
            <?php endwhile; ?>
        <?php else: ?>
            <tr><td colspan="<?= $mode==='borrow'?7:6; ?>">No books found</td></tr>
        <?php endif; ?>
        </tbody>
    </table>
...
<?php elseif($role === 'librarian'): ?>
    ...
    <table class="book-table">
        <thead>
            <tr>
                <th>Book</th><th>Author</th><th>Year</th><th>Publisher</th>
                <th>Borrower</th><th>Due Date</th><th>Borrowed At</th>
            </tr>
        </thead>
        <tbody>
        <?php if($resultBorrowed && $resultBorrowed->num_rows>0): ?>
            ...
            <?php endwhile; ?>
        <?php else: ?>
            <tr><td colspan="7">No borrowed books</td></tr>
        <?php endif; ?>
        </tbody>
    </table>
    ...
    <table class="book-table">
        <thead>
            <tr>
                <th>Book</th><th>Author</th><th>Year</th><th>Publisher</th>
                <th>Borrower</th><th>Due Date</th><th>Returned At</th>
            </tr>
        </thead>
        <tbody>
        <?php if($resultReturned && $resultReturned->num_rows>0): ?>
            ...
            <?php endwhile; ?>
        <?php else: ?>
            <tr><td colspan="7">No returned books</td></tr>
        <?php endif; ?>
        </tbody>
    </table>
<?php endif; ?>
